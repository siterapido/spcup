...
2. `pnpm --filter @spc-up/core test` — 386+ tests pass (source nvm primeiro: `source ~/.nvm/nvm.sh`)
...
2. Wizard: anexar PDF → confirmar **extrato base** (Total) se 2+ Total → mapear colunas + `remetente_destinatario` na sessão
3. `POST .../processar` com `extratoColumnMaps`, `extratoModeloIds`, `arquivoBaseIngestaoId`
4. Planilha: linhas do base; Rem/Dest do PIX pareado; órfãos PIX em painel âmbar na consolidação
