...
from spc_up.services.export.run import export_bundle
from spc_up.services.export.validation import XsdValidationError
...
def export(
    ...
    with session_scope() as session:
        ...
        try:
            paths = export_bundle(session, uf, exercicio, out)
        except ValueError as exc:
            typer.echo(str(exc), err=True)
            raise typer.Exit(code=1) from exc
        except XsdValidationError as exc:
            typer.echo("XML inválido contra XSD SPCA; exportação não publicada.", err=True)
            for filename, errors in exc.errors_by_file.items():
                if not errors:
                    continue
                typer.echo(f"  {filename}:", err=True)
                for message in errors:
                    typer.echo(f"    - {message}", err=True)
            raise typer.Exit(code=1) from exc

    for target in paths:
        if target.name == "validacao.json":
            continue
        typer.echo(f"→ {target}")
