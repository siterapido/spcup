...
async function testSuffixLines(name: string, indices: number[]) {
  const selectedText = indices.map(i => layoutSuffixLines[i]).join("\n");
...
async function main() {
  console.log("Running fast trigger check...");
  
  // Test remaining lines (3, 4, 5, 6, 7)
  const promises = [
    testSuffixLines("Line 3 (historico)", [3]),
    testSuffixLines("Line 4 (situacao)", [4]),
    testSuffixLines("Line 5 (valor)", [5]),
    testSuffixLines("Line 6 (remetente_destinatario)", [6]),
    testSuffixLines("Line 7 (direcao)", [7]),
    testSuffixLines("All lines except 6 (remetente_destinatario)", [0, 1, 2, 3, 4, 5, 7]),
    testSuffixLines("All lines except 5 (valor)", [0, 1, 2, 3, 4, 6, 7])
  ];
