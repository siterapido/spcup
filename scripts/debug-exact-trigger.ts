...
async function testPrompt(name: string, promptText: string) {
  try {
    await execFileAsync("nlm", ["query", "notebook", notebookId, promptText, "--json"]);
...
async function main() {
  // Variant 1: Just layout text prefix
  await testPrompt("Variant 1 (Layout prefix)", basePrompt + `\n\n---\nLayout de colunas informado pelo operador (índice 0 = esquerda). Aplique em todas as páginas deste extrato:\n---`);

  // Variant 2: Adding simple columns without percentage or bracket
  await testPrompt("Variant 2 (Simple columns)", basePrompt + `\n\n---\nLayout de colunas:\ncoluna 0 = data\ncoluna 1 = hora\n---`);

  // Variant 3: Adding column with brackets
  await testPrompt("Variant 3 (Brackets)", basePrompt + `\n\n---\nLayout de colunas:\ncoluna 0 = data (rótulo "Data")\n---`);

  // Variant 4: Adding percentages and ranges
  await testPrompt("Variant 4 (Percent/Ranges)", basePrompt + `\n\n---\nLayout de colunas:\ncoluna 4 = valor [faixa horizontal 81%-100% da página]\n---`);

  // Variant 5: Duplicate colunaIndex mapping
  await testPrompt("Variant 5 (Duplicate Column Index)", basePrompt + `\n\n---\nLayout de colunas:\ncoluna 4 = valor\ncoluna 4 = remetente_destinatario\n---`);

  // Variant 6: Full map suffix but clean
  await testPrompt("Variant 6 (Full layout but clean values)", basePrompt + `\n\n---\nLayout de colunas:\ncoluna 0 = data\ncoluna 1 = hora\ncoluna 2 = historico\ncoluna 3 = situacao\ncoluna 4 = valor\ncoluna 4 = remetente_destinatario\n---`);
}
