...
const promptExtratoJanPix = `Você concilia transações bancárias de prestação de contas partidária no Brasil.
...
const promptExtratoTotalJaneiro = `Você concilia transações bancárias de prestação de contas partidária no Brasil.
Analise apenas o extrato bancário nomeado "b52f881e-64ff-489a-9cc4-9fb5c44f4869_EXTRATO TOTAL JANEIRO (1) (1).pdf" neste notebook. Use os arquivos de cadastro (PF/PJ) e documentos de regras formais do SPCA/TSE contidos neste notebook para cruzamento e classificação.
Extraia todas as transações (lançamentos) de débito e crédito presentes apenas neste extrato. Não inclua transações de outros extratos bancários presentes no notebook.
...
coluna 1 = documento (rótulo "Documento")
coluna 2 = historico (rótulo "Histórico")
coluna 3 = valor (rótulo "Valor") [faixa horizontal 62%-79% da página]
coluna 4 = saldo (rótulo "Saldo") [faixa horizontal 79%-100% da página]
Direção ENTRADA/SAIDA: inferir pelo sinal ou natureza da coluna valor.
...
async function testPrompt(name: string, promptText: string) {
  try {
    ...
    console.log(`[+] ${name}: SUCCESS! Output length: ${stdout.length}`);
  } catch (err: any) {
    console.log(`[-] ${name}: FAILED!`);
    if (err.stdout) console.log("    Stdout:", err.stdout.trim());
    if (err.stderr) console.log("    Stderr:", err.stderr.trim());
  }
}

async function main() {
  await testPrompt("Extrato Jan Pix", promptExtratoJanPix);
  await testPrompt("Extrato Total Janeiro", promptExtratoTotalJaneiro);
}
