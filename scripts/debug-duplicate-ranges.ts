...
async function testPrompt(name: string, suffix: string) {
...
async function main() {
  // Test A: Both with ranges
  await testPrompt("Test A (Both with ranges)", 
    `coluna 4 = valor [faixa horizontal 81%-100% da página]\ncoluna 4 = remetente_destinatario [faixa horizontal 50%-81% da página]`
  );

  // Test B: Both without ranges
  await testPrompt("Test B (Both without ranges)", 
    `coluna 4 = valor\ncoluna 4 = remetente_destinatario`
  );

  // Test C: One with range, one without
  await testPrompt("Test C (One with range, one without)", 
    `coluna 4 = valor [faixa horizontal 81%-100% da página]\ncoluna 4 = remetente_destinatario`
  );
}
