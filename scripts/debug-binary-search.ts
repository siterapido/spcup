...
const basePrompt = `Você concilia transações bancárias de prestação de contas partidária no Brasil.
...
async function testSuffixLines(selectedIndices: number[]): Promise<boolean> {
  const selectedText = selectedIndices.map(i => layoutSuffixLines[i]).join("\n");
  const fullPrompt = basePrompt + "\n\n---\n" + selectedText + "\n---";
  
  try {
    await execFileAsync("nlm", ["query", "notebook", notebookId, fullPrompt, "--json"]);
    return true; // Success
  } catch (err: any) {
    return false; // Failed
  }
...
async function main() {
  console.log("Starting diagnostic binary search on suffix lines...");
  
  // Test complete suffix first to verify it fails
  const allIndices = layoutSuffixLines.map((_, i) => i);
  const allFailed = await testSuffixLines(allIndices);
  console.log(`All suffix lines included: ${allFailed ? "SUCCESS" : "FAILED (expected)"}`);

  // Test line-by-line to see which specific line(s) trigger the failure
  for (let i = 0; i < layoutSuffixLines.length; i++) {
    const success = await testSuffixLines([i]);
    console.log(`Line ${i}: "${layoutSuffixLines[i]}" -> ${success ? "SUCCESS" : "FAILED"}`);
  }
}
